...
class CanonicalMPS:
    ...
    def bond_dims(self) -> list:
        return [l.shape[0] for l in self.lambdas]
...
def mps_decomposition(psi: np.ndarray, p: int = 2, max_bond_dim : int = 0) -> MPS:
    ...
    total_dim = psi.shape[0] if psi.ndim == 1 else np.prod(psi.shape)
    ...
    for site in range(n - 1):
        ...
        left_size = remaining.shape[0] * remaining.shape[1] if remaining.ndim > 1 else remaining.shape[0]
        ...
        # Arbitrary cutoff defined as 1e-12; anything less than that is noise
        d_bond = int(sum(s > 1e-14))

        # D_bond has a minimum value of 1 and is bounded by max_bond_dim
        if max_bond_dim > 0:
            d_bond = min(d_bond, max_bond_dim)
        d_bond = max(d_bond, 1)
...
def mps_to_canonical(mps: MPS) -> CanonicalMPS:
    ...
    for i in range(n - 1, 0, -1):
        ...
        # Reshape to matrix of form (d_left x (p * d_right
        mat = a.reshape(d_left, p_dim * d_right)
